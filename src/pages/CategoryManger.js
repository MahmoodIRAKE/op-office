...
const handleEditclick=(category)=>{
  navigate('/dashboard/category-form', { 
    replace: true,
    state:{category}
  });
}
  const handleDelete= (id)=>{
    CategoriesServices.deleteCategory(setShowLoader,setCategories,setError,id)
  }
  ...
  return (
    <>
      ...
      >
        <MenuItem onClick={()=>handleEditclick(categoryToEdit)}>
          <Iconify icon={'eva:edit-fill'} sx={{ mr: 2 }} />
          Edit
        </MenuItem>

        <MenuItem sx={{ color: 'error.main' }} onClick={()=>handleDelete(categoryToEdit.id)}>
